...
def run_experiment(
    n_trials=50000, sample_sizes=None, k_values=None
) -> tuple[dict[tuple[int, int], dict[str, float]], list[int], range]:
    if sample_sizes is None:
        sample_sizes = [50, 100, 250, 500, 1000, 2500, 5000, 10000, 20000, 40000, 80000]
    if k_values is None:
        k_values = range(2, 6)
...
def debug_single_triple(theta_X, theta_Y0, theta_Y1, n_samples, grid):
    print("\n=== DEBUG SINGLE TRIPLE ===")
    print(f"theta_X={theta_X}, theta_Y0={theta_Y0}, theta_Y1={theta_Y1}, n_samples={n_samples}")

    # Step 1: Generate counts
    counts = generate_counts_fast(n_samples, theta_X, {0: theta_Y0, 1: theta_Y1})
    print("Counts (X->Y order):", counts, "as proportions:", np.array(counts)/n_samples)

    # Step 2: Fit both directions
    params_XY, ll_XY = fit_direction_from_counts(counts, grid, "X->Y")
    params_YX, ll_YX = fit_direction_from_counts(counts, grid, "Y->X")

    print("\nFit results:")
    print(f"X->Y params: {params_XY}, LL_XY={ll_XY}")
    print(f"Y->X params: {params_YX}, LL_YX={ll_YX}")
    print(f"LL diff (XY - YX) = {ll_XY - ll_YX}")

    # Step 3: Reconstruct Y->X counts to check transformation correctness
    ...
    n_X1_Y1 = counts[4]
    n_X0_Y1 = counts[2]
    counts_YX = (n_Y1, n_Y0, n_X1_Y0, n_X0_Y0, n_X1_Y1, n_X0_Y1)
    print("\nCounts transformed for Y->X order:", counts_YX, "as proportions:", np.array(counts_YX)/n_samples)


if __name__ == "__main__":
    np.random.seed(42)

    grid = discretize_params(4)  # pick your k for testing
    debug_single_triple(
        theta_X=0.375,  # example triple
        theta_Y0=0.875,
        theta_Y1=0.125,
        n_samples=500000,
        grid=grid
    ...
    # Uncomment when ready to run the full experiment
