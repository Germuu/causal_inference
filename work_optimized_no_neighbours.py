...
def fit_direction_from_counts(
    ...
    theta_X_hat = n_X1 / (n_X1 + n_X0) if (n_X1 + n_X0) > 0 else 0.0
    theta_Y0_hat = n_Y1_X0 / (n_Y1_X0 + n_Y0_X0) if (n_Y1_X0 + n_Y0_X0) > 0 else 0.0
    theta_Y1_hat = n_Y1_X1 / (n_Y1_X1 + n_Y0_X1) if (n_Y1_X1 + n_Y0_X1) > 0 else 0.0

    # snap to closest grid
    theta_X = grid[np.abs(grid - theta_X_hat).argmin()]
    theta_Y0 = grid[np.abs(grid - theta_Y0_hat).argmin()]
    theta_Y1 = grid[np.abs(grid - theta_Y1_hat).argmin()]

    # compute log-likelihood at the chosen grid point
    probs = [
        theta_X * theta_Y1,       
        theta_X * (1 - theta_Y1),
        (1 - theta_X) * theta_Y0,
        (1 - theta_X) * (1 - theta_Y0)
    ]
    ll = sum(c * np.log(p) for c, p in zip(
        [n_Y1_X1, n_Y0_X1, n_Y1_X0, n_Y0_X0], probs
    ) if c > 0)

    return (theta_X, theta_Y0, theta_Y1), ll
...
def run_experiment(
    ...
    if sample_sizes is None:
        sample_sizes = [50, 100, 250, 500, 1000, 2500, 5000, 10000, 20000, 40000, 80000, 160000, 320000, 640000, 1280000, 2560000, 5120000, 10240000]
    if k_values is None:
        k_values = range(2, 10)
